...
**Version:** 2.0.0 **Date:** 2025-11-14 **Status:** ✅ Implementation Complete | ⏳ Integration Pending
...
* Added auth-specific errors (INVALID\_LINK, INSUFFICIENT\_PERMISSIONS, etc.)
...
#### **merchant\_schema.sql** (Previously implemented)

* Added `mode` field to `merchants` table ('payment', 'authentication', 'both')
* Created `user_merchant_links` table with:
  * Dual-mode support (link\_type: 'payment' or 'authentication')
  * Payment fields: payment\_provider, payment\_token\_reference
  * Auth fields: permissions\[], scopes\[]
  * Tracking: total\_verifications, successful\_verifications, fraud\_score
  * Soft delete: deleted\_at timestamp
...
**Current State:** All endpoints have TODO comments with SQL queries:
...
* [ ] Environment variables configured (JWT\_SECRET, DATABASE\_URL)
...
**Last Updated:** 2025-11-14 **Version:** 2.0.0 **Status:** Implementation Complete | Integration Pending
