...
**Deployment ID:** `282caa9b-98f9-4ae5-b247-f8c9388d110e` **Status:** ✅ SUCCESS **Date:** 2025-12-27 19:20 UTC **Commit:** `a386f7a`
...
**Issue:** ensureAliasViews() may be failing to create required views/tables **Impact:** Non-critical - alias statistics not available **Fix:** Needs investigation of wrapped\_keys table schema
...
**Issue:** Database tables not created **Impact:** Crypto payment configuration unavailable **Fix:** Need to run migration 010\_add\_crypto\_payments.sql
...
* **Root Cause:** Database views (alias\_word\_stats, alias\_usage\_stats) may not be created
...
* **Error:** "relation 'crypto\_supported\_chains' does not exist"
...
| Audit logs "created\_at" error    | Changed to use `timestamp` column       | `3a8876e` |
| Schema initialization failure     | Reverted audit\_log to match production | `a386f7a` |
...
2. **Run crypto migrations** - Create crypto\_supported\_chains and crypto\_supported\_tokens tables
